...
func testOrdersExists(t *testing.T) {
	...
	e, err := OrderExists(ctx, tx, o.UserID, o.ProductID)
	if err != nil {
...
func testOrdersFind(t *testing.T) {
	...
	orderFound, err := FindOrder(ctx, tx, o.UserID, o.ProductID)
	if err != nil {
		t.Error(err)
	}
...
func testOrderToOneSetOpProductUsingProduct(t *testing.T) {
	...
	for i, x := range []*Product{&b, &c} {
		...
		if exists, err := OrderExists(ctx, tx, a.UserID, a.ProductID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	orderDBTypes = map[string]string{`UserID`: `int`, `ProductID`: `int`, `Quantity`: `int`}
	_            = bytes.MinRead
)
