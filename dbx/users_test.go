...
func testUserToManyReceipts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, receiptDBTypes, false, receiptColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, receiptDBTypes, false, receiptColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Receipts().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadReceipts(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Receipts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Receipts = nil
	if err = a.L.LoadReceipts(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Receipts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpReceipts(t *testing.T) {
	var err error
	...
	foreigners := []*Receipt{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, receiptDBTypes, false, strmangle.SetComplement(receiptPrimaryKeyColumns, receiptColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Receipt{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddReceipts(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Receipts[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Receipts[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Receipts().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `DeletedAt`: `timestamp`, `Name`: `varchar`, `Email`: `varchar`, `Password`: `varchar`, `Avatar`: `varchar`, `Points`: `double`}
	_           = bytes.MinRead
)
